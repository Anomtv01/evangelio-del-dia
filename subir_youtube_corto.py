# -*- coding: utf-8 -*-
"""
Sube el Short diario del Evangelio a YouTube usando la YouTube Data API v3.

Reutiliza las MISMAS credenciales de Google que el resto del canal
(client_secret.json / token.json en la raíz). Mismo canal, playlist propia
para los Shorts.

Uso:
    python subir_youtube_corto.py output_corto/corto_2026-08-25.json
"""
...
NOMBRE_PLAYLIST = "Evangelio en 2 Minutos"
...
def buscar_o_crear_playlist(youtube, nombre):
    ...
    body = {
        "snippet": {"title": nombre,
                    "description": "Un Short cada día con una historia del Evangelio."},
...
def ya_existe_video(youtube, titulo):
    """Evita subir dos veces el mismo Short si el workflow corre dos veces
    el mismo día (reintento de GitHub, Run workflow manual, etc.)."""
    try:
...
def subir_video(video_path, titulo, descripcion, tags):
    ...
    print(f"  Short subido: https://youtube.com/shorts/{video_id}")
    try:
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python subir_youtube_corto.py output_corto/corto_<fecha>.json")
        sys.exit(1)
    ...
    if not video_path or not os.path.exists(video_path):
        print(f"[ERROR] No se encontró el video (_video_path: {video_path}). "
              f"¿Corriste generar_video_corto.py?")
        sys.exit(1)
    for campo in ("titulo", "descripcion", "tags"):
        if campo not in data:
            print(f"[ERROR] Falta '{campo}'. ¿Corriste generar_metadata_corto.py?")
            sys.exit(1)
    ...
    if video_id is None:
        print("No se subió nada (ya existía un Short con ese título hoy).")
        sys.exit(0)

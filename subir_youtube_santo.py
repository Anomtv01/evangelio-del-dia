# -*- coding: utf-8 -*-
"""
Sube el video del Santo del Día a YouTube usando la YouTube Data API v3.

Reutiliza las MISMAS credenciales de Google que ya usa el Evangelio del Día
(client_secret.json / token.json en la raíz del proyecto) — es el mismo
canal de YouTube, solo cambia la playlist de destino.

A diferencia de subir_youtube.py (Evangelio), acá:
  - El video y la metadata NO están en archivos separados; ya vienen
    adentro del mismo JSON que fue pasando por todo el pipeline
    (santo_del_dia.py -> generar_metadata_santo.py -> generar_video_santo.py),
    bajo las claves "_video_path", "titulo", "descripcion", "tags".
  - La playlist de destino es "Santo del Día" (podés cambiar el nombre
    en NOMBRE_PLAYLIST más abajo).

Uso:
    python subir_youtube_santo.py output_santo/santo_2026-08-11.json
"""
...
# Mismas credenciales que el Evangelio del Día (mismo canal de YouTube).
...
# Nombre EXACTO de la playlist en YouTube. Si no existe, se crea sola.
NOMBRE_PLAYLIST = "Santo del Día"
...
def buscar_o_crear_playlist(youtube, nombre_playlist):
    ...
    body = {
        "snippet": {
            ...
            "description": "Historias breves de la vida de los santos, cada día.",
        },
...
def subir_video(video_path, titulo, descripcion, tags):
    ...
    if ya_existe_video(youtube, titulo):
        print(f"  [!] YA EXISTE un video con el titulo '{titulo}'. No se sube de nuevo.")
        return None
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python subir_youtube_santo.py output_santo/santo_<fecha>.json")
        sys.exit(1)
    ...
    if not video_path or not os.path.exists(video_path):
        print(f"[ERROR] No se encontró el video generado (_video_path: {video_path}). "
              f"¿Corriste antes generar_video_santo.py?")
        sys.exit(1)

    for campo in ("titulo", "descripcion", "tags"):
        if campo not in data:
            print(f"[ERROR] Falta '{campo}' en el JSON. "
                  f"¿Corriste antes generar_metadata_santo.py?")
            sys.exit(1)
